...
def tunnel(source_playlist_name, source, destination, core_sessions):
   ...
   if source == "spotify":
      ...
      print(f"[-]: {source} is an unrecognized source. Use 'spotify', 'tidal' or 'youtube'")
      sys.exit(1)
   ...
   if destination == "youtube":
      ...
      print(f"[-]: {destination} is an unrecognized destination. Use 'spotify', 'tidal' or 'youtube'")
      sys.exit(1)
...
def main():
   ...
   if args.source == args.destination:
      print(f"[-]: Nice try but no you can't move from {args.source} to {args.source}, they are the same platform")
      sys.exit(1)
   ...
   if args.L:
      if args.source == "spotify":
         ...
         print(f"-s {args.source} is unrecognized. Use '-s youtube', '-s spotify' etc")
         sys.exit(0)
   elif args.p:
