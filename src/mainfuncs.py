...
def confirm_playlist_exist(source_playlist_name, plat_list, platform=""):
    ...
    if source_playlist_name in plat_list:
        return plat_list[source_playlist_name]

    # Try with trailing space
    if source_playlist_name + " " in plat_list:
        print(
            f"[DEBUG] Found playlist with trailing space: '{source_playlist_name + ' '}'"
        )
        return plat_list[source_playlist_name + " "]
    ...
    if source_playlist_name.rstrip() in plat_list:
        print(
            f"[DEBUG] Found playlist without trailing space: '{source_playlist_name.rstrip()}'"
        )
        return plat_list[source_playlist_name.rstrip()]
...
def report_sync_summary(total_not_found):
    ...
    if total_not_found == 0:
        ...
        print(
            "[i] Check 'notfound.txt' for details about tracks that couldn't be transferred."
        )
